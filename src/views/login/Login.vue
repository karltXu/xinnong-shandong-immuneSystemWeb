    ...
    <a-form id="formLogin" class="user-layout-login" ref="formLogin" :form="form" @submit="handleSubmit">
      ...
      <a-form-item>
        <a-checkbox v-decorator="['rememberMe', { valuePropName: 'checked' }]">
          <a :href="config" target="_blank" class="forge-password">
            本人已阅读并同意使用条款
          </a>
        </a-checkbox>
      </a-form-item>
...
import { getSmsCaptcha, getConfig } from '@/api/login'
...
export default {
  ...
  data() {
    return {
      ...
      config: 'https://assets.aibcxw.com/user-content/docs/termsof-service.html'
    }
  },
  created() {
    // this.requiredTwoStepCaptcha = true
    getConfig().then((res) => {
      this.config = res.data.termsOfServiceURL
    })
  },
  methods: {
      ...
      validateFields(validateFieldsKey, { force: true }, (err, values) => {
        if (!err) {
          ...
          if (!loginParams.rememberMe) {
            this.$message.warning('请您先阅读并勾选服务条款')
