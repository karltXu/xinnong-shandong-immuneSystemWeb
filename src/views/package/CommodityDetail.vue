    ...
    <a-card :bordered="false">
      <a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
        ...
        <a-form-model-item label="商品描述" prop="description">
          <a-textarea :rows="5" v-model="form.description" />
        </a-form-model-item>

        <a-form-model-item label="商品价格(元)" prop="price">
...
<script>
import { getCommodityDetail, CommodityUpdate } from '@/api/package'
...
export default {
  ...
  mounted() {
    getCommodityDetail(this.$route.params.id).then(res => {
      this.form = res.data.commodity
